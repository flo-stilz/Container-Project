Feature: Client Management
  ...
  Scenario: Register clients
    Given a client "client1" with company name "Matas" address "DRbyen" email "matas@administration.com" contact person "Signe Nielsen"
    And a second client "client2" with company name "Miele" address "Dortmund" email "Miele@administration.du" contact person "Karl-Heinz Rummenigge"
  ...
  Scenario: Checking generated clientid
    Given a client "client2" with company name "Oerstad" address "Fredericia" email "Oerstad@administration.com" contact person "Henrik Poulsen"
    And a second client "client4" with company name "Coloplast" address "Humlebaek" email "Coloplast@administration.com" contact person "Anette Mikkelsen"
  ...
  Scenario: Register the same client twice
    Given a client "client5" with company name "Sats" address "Lyngby" email "Sats@administration.com" contact person "Robert Stork"
  ...
  Scenario: Update client information
     Given a client "client6" with company name "Sats" address "Lyngby" email "Sats@administration.com" contact person "Robert Stork"
  ...
  Scenario: Search for a client with a specific email email
     Given a client "client7" with company name "Tesla" address "Pao Alto, California" email "Tesla@administration.com" contact person "Travis Fimmel"
     And a second client "client8" with company name "Dansk Metal" address "Aarhus" email "DanskMetal@administration.com" contact person "Morten Pedersen"
  ...
  Scenario: Search for a client that is not present in the clientlist using a specific address
     Given a client "client9" with company name "IBM" address "Armonk New York, United States" email "IBM@administration.com" contact person "Arvind Krishna"
     And a second client "client10" with company name "ICBC" address "Xicheng District, Beijing, China" email "ICBC@administration.com" contact person ""
