Feature: xxxxxxpersistency
  ...
  Scenario: Storing two clients
    ...
    And a client "client8" with company name "Nike" address "madisonsquaregarden" email "nike@administration.com" contact person "Peter Jackson" password "peter"
  ...
  Scenario: Restoring a client when client information is updated
    Given Journey and container counter is set to zero, where the counter describes the name of the corresponding id's
    And a client "client10" with company name "Nike" address "madisonsquaregarden" email "nike@administration.com" contact person "Peter Jackson" password "peter"
