...
public class stepDef {
	...
	public void the_client_is_registered_in_the_client_database(String cid) {
		clients.put(cid, database.createClient(companyName, address, mail, name, password));
	}
	...
	public void the_second_client_is_registered_in_the_client_database(String cid) {
		clients.put(cid, database.createClient(companyName2, address2, mail2, name2, password2));
	}
	...
	public void a_unique_journey_id_is_created() {
		assertEquals(database.getJourney().get(0).getId(), "CPHRIO26");
	}
	...
	public void the_journeys_current_location_is_updated() {
		database.updateCurrentLocation(database.getJourney().get(0), newloc);
	}
    ...
    public void the_journey_is_completed(String jid) {
    	database.updateCurrentLocation(journeys.get(jid), journeys.get(jid).getDestination());
	...
	public void the_container_for_the_old_journey_is_reused_for_the_new_journey() {
		assertEquals(database.getHistory().get(0).getContainerList().get(0).getContainerId(), "C0");
		assertEquals(database.getJourney().get(0).getContainerList().get(0).getContainerId(), "C0");
	}
	...
	public void searching_for_the_container_containing_the_content(String keyword) {
		...
		res = database.findContainer(keyword, database.getJourney());	
	}
	...
	public void filtering_for_all_the_active_containers() {
		filteredcontainer = database.getfilteredContainers(true, database.getJourney());
	}
	...
	public void a_list_of_all_the_active_containers_is_returned() {
		...
		assertEquals(filteredcontainer.get(0).getContainerId(), database.getJourney().get(0).getContainerList().get(0).getContainerId());
	}
	...
	public void filtering_for_all_containers_in_the_containerwarehouse() {
		filteredcontainer = database.getfilteredContainers(false, database.getJourney());
	}
	...
	public void all_containers_have_been_retrieved(){
		allcontainers = database.getAllContainers(false, database.getJourney());
	}
	...
	public void a_client_is_created_based_on_the_simulated_information(String cid) {
		...
		clients.put(cid, database.createClient(company, address, mail, name, password));
	}
	...
	public void the_journey_exists_in_the_database() {
	    assertEquals(database.getJourney().size(), 1);
	    assertEquals(database.getJourney().get(0).getId(),"CPHRIX10");
	}
    ...
    public void the_journey_history_of_container_should_be_returned(String search) {
	...
	public void the_active_clients_are_present_in_the_Clients_xml() {
		System.out.println("yay" + database.readClientFile());
